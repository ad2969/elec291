...
volatile unsigned int ISR_pw_top=0, ISR_pw_bottom=0,ISR_cnt_top=0, ISR_cnt_bottom=0, ISR_frc;
...
void SysInit(void)
{
	...
	// Set up timer
	RCC_APB1ENR |= BIT1; // turn on clock for timer1
	TIM3_ARR = SYSCLK/DEF_F;
	ISER |= BIT16;        // enable timer interrupts in the NVIC
	TIM3_CR1 |= BIT4;     // Downcounting
	TIM3_CR1 |= BIT0;     // enable counting
	TIM3_DIER |= BIT0;    // enable update event (reload event) interrupt
	enable_interrupts();
